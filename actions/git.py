...
class Git:
    def __init__(self, org, project, credentials, date):
        self.org = org  #cfg['org']
        self.project = project  #cfg['project']
        self.credentials = credentials  #cfg['credentials']
        self.date = date  #cfg['date']


    def make_local(self, student):
        return f'{self.project}-{student}'


    def make_remote(self, student):
        if self.credentials == 'ssh':
            return f'git@github.com:/{self.org}/{self.project}-{student}'
        else:
            return f'https://github.com/{self.org}/{self.project}-{student}'
    ...
    def get_commit_hash(self, local, branch):
        ...
        time = '' if ' ' in self.date else ' 00:00:00'
